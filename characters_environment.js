...
// change game status if any pose is taken place
function startGame(){
	Gamestatus="start";
	document.getElementById("load").innerHTML="Game is loading";
}
function changeGameStatud(character){
  if(Gamestatus=="start"&& noseX!=""&& gameConfig.status==="start") 
  {
    document.getElementById("load").innerHTML="game is loaded"
    world_start.play();
